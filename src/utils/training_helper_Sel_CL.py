...
def train_eval_model(model, x_train, x_test, Y_train, Y_test, Y_train_clean,
    ...
    for epoch in range(args.epochs):
        model.train()
        for batch_idx, (data, target, indices, y_clean) in enumerate(train_loader):
            ...
            with autocast():
                output = model(data)
                uns_loss = uns_contrast(output, target)
                loss = criterion(output, target) + args.lambda_s * uns_loss

            # Scale loss and perform backward pass
            scaler.scale(loss).backward()

            # Update optimizer and scale gradients
    ...
    test_results = dict()
    test_results['acc'] = test_accs[-1]
    test_results['f1_weighted'] = test_f1s[-1]
