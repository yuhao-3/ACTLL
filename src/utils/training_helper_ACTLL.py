    ...
    try:
        ...
        for e in range(args.epochs):
            ...
            if e <= args.warmup:
                ...
                train_accuracy, avg_loss, model_new, confident_set_id = train_step_ACTLL(
...
def main_wrapper_ACTLL(args, x_train, x_test, Y_train_clean, Y_test_clean, saver,seed=None):
    ...
    elif args.AEChoice == 'Inception':
        model = InceptionTemporalAE(input_size=x_train.shape[2], num_filters=args.filters, embedding_dim=args.embedding_size,
                                     seq_len=x_train.shape[1], kernel_sizes=[3,5,7,9], num_modules=4,kernel_size = args.kernel_size,
                                     stride=args.stride, padding=args.padding, dropout=args.dropout)
...
def train_step_ACTLL(data_loader, model, loss_centroids, optimizer, criterion, yhat_hist, loss_all=None, epoch=0, args=None, sel_dict=None):
    ...
    for batch_idx, (x, y_hat, x_idx, _) in enumerate(data_loader):
        ...
        assert not torch.isnan(model_loss).any(), "NaN detected at this stage"
        model_loss.backward()
        torch.nn.utils.clip_grad_norm_(model.parameters(), 5.0)
        optimizer.step()
