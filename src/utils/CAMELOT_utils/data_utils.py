...
class CustomDataset(Dataset):
    ...
    def create_transition_matrix(self, dim, noise_level):
        # Ensure noise level is appropriate
        if noise_level < 0 or noise_level > 1:
            raise ValueError("Noise level must be between 0 and 1")
        
        # The diagonal values (true class probabilities)
        true_class_prob = 1 - noise_level
        
        # Create an initial matrix filled with noise_level divided by (dim - 1) for off-diagonal elements
        transition_matrix = np.full((dim, dim), fill_value=(noise_level / (dim - 1)))
        
        # Set the diagonal values to true_class_prob
        np.fill_diagonal(transition_matrix, true_class_prob)
        
        return transition_matrix
    
    def add_label_noise(self, y, noise_level):
        dim = y.shape[1]
        # Define transition probabilities
        transition_matrix = self.create_transition_matrix(dim, noise_level)
        
        # Get the indices of the original labels
        original_labels = np.argmax(y, axis=1)
        
        # Generate noisy labels based on the transition matrix probabilities
        noisy_labels = np.array([
            np.random.choice(range(dim), p=transition_matrix[label])
            for label in original_labels
        ])
        
        # Return noisy labels in one-hot encoded form
        return np.eye(dim)[noisy_labels]  # Convert to one-hot encoding
    ...
    def _convert_to_3d_arr(self, X):
        ...
        print(X.groupby(self.id_col).count())
        print(feats)
